...
RUN -i sed -i 's/$db_user="usuario"/$db_user="$user"/g' var.php
RUN -i sed -i 's/$db_host="localhost"/$db_host="$host"/g' var.php
RUN -i sed -i 's/$db_password="2asirtriana"/$db_user="$contra"/g' var.php
RUN -i sed -i 's/$db_name="alebuntu"/$db_user="$gbd"/g' var.php
